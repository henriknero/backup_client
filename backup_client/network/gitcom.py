...
def create_new_repository(path, credentials):
    ...
    response = req.post(API_URL, data={'name':repo_name, 'private':True}, auth=credentials)
...
def commit_and_push_all(repository, credentials):
    if repository.status(): #pylint: disable=E1101
        ...
        request = req.get('https://nerobp.xyz/gogs/api/v1/users/' + credentials[0], auth=credentials)
        ...
        repository.remotes[0].push([ref], RemoteCallbacks(UserPass(credentials[0], credentials[1])))
