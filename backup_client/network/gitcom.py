...
GIT_SERVER = 'https://nerobp.xyz/gogs/'
...
def add_remote_repository(path, repo_name, credentials):
    ...
    if all([credentials[0], credentials[1]]):
        ...
        return clone_repository(os.path.join(GIT_SERVER, credentials[0], repo_name), path, callbacks=callbacks)

    return clone_repository(os.path.join(GIT_SERVER, credentials[0], repo_name), path)
